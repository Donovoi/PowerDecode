...
function IsBase64() {
    ...
    #Remove Ticks
	$InputString = RemoveTicks($InputString)
...
function DecodeBase64() {
	...
	#Remove Ticks
	$StringBase64 = RemoveTicks($StringBase64)
...
<#
Function name: DecodeGzipFull
Description: Receives a gzip compression command as input, extracts the payload and removes base64 compression gzip encoding from it     
Function calls: GoodSyntax
Input: $InputString
Output: $DecodedPayload
#>
function DecodeGzipFull() {
	param(
        [Parameter(
			Mandatory = $True)]
        [ string ] $InputString
...
$InputString = $InputString.replace(" ","").replace("`n","").replace("`r","");
$CompressedRegex =@"
[Ff][Rr][Oo][Mm][Bb][Aa][Ss][Ee][6][4][Ss][Tt][Rr][Ii][Nn][Gg](.+)\.[Rr][Ee][Aa][Dd][Tt][Oo][Ee][Nn][Dd](\s*)(\()(\s*)(\))
"@

	$CompressedPattern = [regex] $CompressedRegex
    $CompressedMatches = $CompressedPattern.Matches($InputString )
	
	$Base64Command = ""
	$CompressedCommand = ""
	
	foreach ($match in $CompressedMatches) {
	 $Base64Command =  $match.value
	 $CompressedCommand = "(New-Object System.IO.StreamReader(New-Object System.IO.Compression.GzipStream((New-Object System.IO.MemoryStream(,[System.Convert]::"+$Base64Command
	
	}

if (!(GoodSyntax  $CompressedCommand) ) {		
		Write-Host "Compression command contains some syntax errors, trying to extract payload" -ForegroundColor red
		$Base64Command = ExtractCompressedPayload $CompressedCommand
		$DecodedPayload  = DecodeGzip $Base64Command
}

else {	
	try{
		$DecodedPayload = IEX ( $CompressedCommand )
	}
	catch{
		Write-Host "Exception raised while decoding gzip payload:" -ForegroundColor red -nonewline
		Write-Host $Error[0]	
	}
