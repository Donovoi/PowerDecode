...
function DeobfuscateByRegex 
	{
       ...
       While( $NewScript -ne $OldScript )
       {
			...
			$TempScript = ConcatenateCleanup3($NewScript )
			if (GoodSyntax $TempScript) { $NewScript = $TempScript  }
...
<#
Function name: AdaptCode
Description: Receives as input a script and removes the Powershell call header 
Function calls: GoodSyntax, RemovePowerShellCall
Input:  $Script 
Output: $NewScript
#>
function AdaptCode
