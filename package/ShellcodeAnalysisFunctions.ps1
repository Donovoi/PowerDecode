...
if (IsShellcodeSREBase64Encoded $Script) {
$data += DisassembleSREBase64String $Script
}


if(IsShellcodeSoapHexBinaryEncoded $Script){
$data += DisassembleSoapHexBinaryString $Script
}
...
<#
Function name: IsShellcodeSoapHexBinaryEncoded
Description: Recognizes Soap Hex Binary encoded shellcode
Function calls: -
Input:  $Script
Output: $true/false
#>
function IsShellcodeSoapHexBinaryEncoded()  {
	 param(
            [Parameter( 
                Mandatory=$True, 
                Valuefrompipeline = $True)]
...
<# example:
E0ED2393969393939793939392929393
#>

$PayloadRegex = "[A-Fa-f0-9]{3200,}"; #detect payload
$FormatRegex = "\[System\.Runtime\.Remoting\.Metadata\.W3cXsd2001\.SoapHexBinary\]\:\:Parse"; #detect format
$ConversionRegex = @"
\[System\.BitConverter\]\:\:GetBytes\((\$)([A-Za-z0-9]+)\[(\$)([A-Za-z0-9]+)\](\s* )\-(\s*)(\d+)(\s*\))\[\d+\]
"@  #detect conversion

$PayloadPattern = [regex] $PayloadRegex
$PayloadMatches = $PayloadPattern.matches($Script)

$FormatPattern = [regex] $FormatRegex
$FormatMatches = $FormatPattern.matches($Script)

if ( ($PayloadMatches.count -gt 0)  -and ($FormatMatches.count -gt 0  ) ) {
	return $true ;
}
...
<#
Function name: DisassembleSoapHexBinaryString
Description: Analyzes Soap Hex Binary encoded shellcode and returns the disassembled code
Function calls: GetAssemblyInstructions, GetSCDBGReport
Input:  $Script
Output: $data
#>
function DisassembleSoapHexBinaryString()  {
        param(
            [Parameter( 
                Mandatory=$True, 
                Valuefrompipeline = $True)]
...
$ConversionPattern = [regex] $ConversionRegex
$ConversionMatches = $ConversionPattern.matches($Script)

$OffsetPattern = [regex] $OffsetRegex

$Payload="";
foreach($match in $PayloadMatches){
$Payload = $match.value
}
...
$Conversion="";
$Offset="";
foreach($match in $ConversionMatches){
$Conversion = $match.value

$OffsetMatches = $OffsetPattern.matches($Conversion)
foreach ($match2 in $OffsetMatches){
$Offset = ($match2.value).replace(" ","").replace("-","");	
...
#Decoding
$SoapHexBinaryPayload = [System.Runtime.Remoting.Metadata.W3cXsd2001.SoapHexBinary]::Parse($Payload).Value;
if ($ConversionMatches.count -gt 0) {
	$o = [int]$Offset
	For ($i=0; $i -lt $SoapHexBinaryPayload.Length; $i++) {$SoapHexBinaryPayload[$i] = [System.BitConverter]::GetBytes($SoapHexBinaryPayload[$i] - $o)[0];}	
}

#Disassembling
	$HexString = ($SoapHexBinaryPayload|ForEach-Object ToString X2) -join ' '
	$HexString = $HexString.replace(" ","")
	if($HexString.length -ge 32){
			
		$assembly = Format-Hex -InputObject $SoapHexBinaryPayload
		$AssemblyInstructions =  (GetAssemblyInstructions -Architecture X86 -Mode Mode16 -Code $SoapHexBinaryPayload -Offset 0x1000) | Out-String 

		$data += "Script attempts to inject shellcode into memory; Assembly instructions are encoded via SoapHexBinary" + "`n`n" + $assembly
		$data += "`n`n" +"Assembly instructions:"+$AssemblyInstructions +"`n`n"
		$data += GetSCDBGReport $HexString
...
<#
Function name: IsShellcodeSREBase64Encoded
Description: Recognizes base 64 encoded shellcode invoked via System Reflection Assembly
Function calls: -
Input:  $Script
Output: $true/false
#>
function IsShellcodeSREBase64Encoded()  {
	 param(
            [Parameter( 
                Mandatory=$True, 
                Valuefrompipeline = $True)]
...
<# example:
Lx73vBc9/a+v99f/5uKvMPJyofd
#>

$PayloadRegex = "[A-Za-z0-9\\\/\+\=]{3200,}"; #detect payload
$FormatRegex = "\[system\.convert\]\:\:frombase64string"; #detect format
$ConversionRegex = @"
...
$FormatPattern = [regex] $FormatRegex
$FormatMatches = $FormatPattern.matches( $Script.toLower() )

$ConversionPattern = [regex] $ConversionRegex
$ConversionMatches = $ConversionPattern.matches( $Script.toLower() )

if ( ($PayloadMatches.count -gt 0)  -and ($FormatMatches.count -gt 0  ) -and ($ConversionMatches.count -gt 0 ) ) {
	return $true ;
}
...
<#
Function name: DisassembleSREBase64String
Description: Analyzes base 64 encoded shellcode (invoked via System Reflection Assembly ) and returns the disassembled code
Function calls: GetAssemblyInstructions, GetSCDBGReport
Input:  $Script
Output: $data
#>
function DisassembleSREBase64String()  {
        param(
            [Parameter( 
                Mandatory=$True, 
                Valuefrompipeline = $True)]
...
#Disassembling
	...
	if($HexString.length -ge 32){
		...
		$data += "Script attempts to inject shellcode into memory; Assembly instructions are base64 encoded (invoked via System.Reflection.Assembly) " + "`n`n" + $assembly
		$data += "`n`n" +"Assembly instructions:"+$AssemblyInstructions +"`n`n"
		$data += GetSCDBGReport $HexString
